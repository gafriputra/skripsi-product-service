...
func (r *repository) GetProducts(limit int, offset int) ([]Product, error) {
	...
	err := r.db.Preload("Galleries").Limit(limit).Offset(offset).Find(&products).Error
	return products, err
}

func (r *repository) GetProduct(slug string) (Product, error) {
	var product Product
	err := r.db.Preload("Galleries").Preload("Documents").Where("slug = ?", slug).Find(&product).Error
	return product, err
}
